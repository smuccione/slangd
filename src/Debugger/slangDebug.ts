...
import { slangRuntime, IslangBreakpoint, FileAccessor } from './slangRuntime';
...
/**
 * This interface describes the slang-debug specific launch attributes
 * (which are not part of the Debug Adapter Protocol).
 * The schema for these attributes lives in the package.json of the slang-debug extension.
...
export class slangDebugSession extends LoggingDebugSession {

	// we don't support multiple threads, so we can use a hardcoded ID for the default thread
	private static threadID = 1;

	// a slang runtime (or debugger)
	private _runtime: slangRuntime;
	...
	public constructor(fileAccessor: FileAccessor) {
		...
		this._runtime = new slangRuntime(fileAccessor);

		// setup event handlers
		this._runtime.on('stopOnEntry', () => {
			this.sendEvent(new StoppedEvent('entry', slangDebugSession.threadID));
		});
		this._runtime.on('stopOnStep', () => {
			this.sendEvent(new StoppedEvent('step', slangDebugSession.threadID));
		});
		this._runtime.on('stopOnBreakpoint', () => {
			this.sendEvent(new StoppedEvent('breakpoint', slangDebugSession.threadID));
		});
		this._runtime.on('stopOnDataBreakpoint', () => {
			this.sendEvent(new StoppedEvent('data breakpoint', slangDebugSession.threadID));
		});
		this._runtime.on('stopOnException', (exception) => {
			if (exception) {
				this.sendEvent(new StoppedEvent(`exception(${exception})`, slangDebugSession.threadID));
			} else {
				this.sendEvent(new StoppedEvent('exception', slangDebugSession.threadID));
			}
		});
		this._runtime.on('breakpointValidated', (bp: IslangBreakpoint) => {
...
//				supportsCondition: false
	...
	protected async setExceptionBreakPointsRequest(response: DebugProtocol.SetExceptionBreakpointsResponse, args: DebugProtocol.SetExceptionBreakpointsArguments): Promise<void> {
		const namedException: string | undefined = undefined;
		let otherExceptions = false;
/*

		if (args.exceptionOptions) {
			for (const filterOption of args.exceptionOptions) {
				switch (filterOption.filterId) {
					case 'namedException':
						namedException = args.exceptionOptions[0].condition;
						break;
...
*/
	...
	protected threadsRequest(response: DebugProtocol.ThreadsResponse): void {
		...
		response.body = {
			threads: [
				new Thread(slangDebugSession.threadID, "thread 1")
			]
		};
		this.sendResponse(response);
	}
	...
	private async progressSequence() {
		...
		const startEvent: DebugProtocol.Event = new ProgressStartEvent(ID, title);
	...
	private createSource(filePath: string): Source {
		return new Source(basename(filePath), this.convertDebuggerPathToClient(filePath), undefined, undefined, 'slang-adapter-data');
	}
}
