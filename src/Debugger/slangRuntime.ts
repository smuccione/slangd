...
export interface IslangBreakpoint {
	id: number;
	line: number;
	verified: boolean;
}
...
/**
 * A slang runtime with minimal debugger functionality.
 */
export class slangRuntime extends EventEmitter {
	...
	// maps from sourceFile to array of slang breakpoints
	private _breakPoints = new Map<string, IslangBreakpoint[]>();
	...
	/**
	 * "Step into" for slang debug means: go to next character
	 */
	...
	/**
	 * "Step out" for slang debug means: go to previous character
	 */
	public stepOut() {
		if (typeof this._currentColumn === 'number') {
			this._currentColumn -= 1;
	 ...
	 */
	public async setBreakPoint(path: string, line: number): Promise<IslangBreakpoint> {

		const bp: IslangBreakpoint = { verified: false, line, id: this._breakpointId++ };
		let bps = this._breakPoints.get(path);
		if (!bps) {
			bps = new Array<IslangBreakpoint>();
	 ...
	 */
	public clearBreakPoint(path: string, line: number): IslangBreakpoint | undefined {
